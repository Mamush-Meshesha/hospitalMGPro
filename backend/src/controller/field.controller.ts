...
export const getAll = async (req: Request, res: Response) => {
  try {
    const results = await FieldService.getAll();
...
export const getById = async (req: Request, res: Response) => {
  try {
    const result = await FieldService.getById(req.params.id);
...
export const create = async (req: Request, res: Response) => {
  try {
    const result = await FieldService.create(req.body);
    const v = req.query.v as string || 'default';
...
export const update = async (req: Request, res: Response) => {
  try {
    const result = await FieldService.update(req.params.id, req.body);
    const v = req.query.v as string || 'default';
    res.status(200).json(RepresentationEngine.format(result, v));
  } catch (error: any) {
    res.status(500).json({ error: error.message });
