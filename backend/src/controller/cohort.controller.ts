...
export const getAll = async (req: Request, res: Response) => {
  try {
    const results = await CohortService.getAll();
...
export const getById = async (req: Request, res: Response) => {
  try {
    const result = await CohortService.getById(req.params.id);
...
export const update = async (req: Request, res: Response) => {
  try {
    const result = await CohortService.update(req.params.id, req.body);
    const v = req.query.v as string || 'default';
    res.status(200).json(RepresentationEngine.format(result, v));
  } catch (error: any) {
    res.status(500).json({ error: error.message });
...
export const remove = async (req: Request, res: Response) => {
  try {
    await CohortService.remove(req.params.id);
    res.status(204).send();
  } catch (error: any) {
    res.status(500).json({ error: error.message });
  }
