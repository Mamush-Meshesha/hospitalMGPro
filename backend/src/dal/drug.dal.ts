...
export class DrugDAL {
  static async getAll() {
    return await prisma.drug.findMany({ take: 50 });
  }

  static async getById(id: string) {
    return await prisma.drug.findFirst({ where: { uuid: id } });
  }
  ...
  static async update(id: string, data: any) {
    return await prisma.drug.updateMany({ where: { uuid: id }, data });
  }

  static async remove(id: string) {
    // Soft Delete (Metadata)
    return await prisma.drug.updateMany({ 
      where: { uuid: id }, 
