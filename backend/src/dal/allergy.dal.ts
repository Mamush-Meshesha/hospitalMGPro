...
export class AllergyDAL {
  static async getAll() {
    return await prisma.allergy.findMany({ take: 50 });
  }

  static async getById(id: string) {
    return await prisma.allergy.findFirst({ where: { uuid: id } });
  }
  ...
  static async update(id: string, data: any) {
    return await prisma.allergy.updateMany({ where: { uuid: id }, data });
  }

  static async remove(id: string) {
    // Soft Delete (Clinical Data)
    return await prisma.allergy.updateMany({ 
      where: { uuid: id }, 
