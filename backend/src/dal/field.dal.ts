...
export class FieldDAL {
  static async getAll() {
    return await prisma.field.findMany({ take: 50 });
  }

  static async getById(id: string) {
    return await prisma.field.findFirst({ where: { uuid: id } });
  }
  ...
  static async update(id: string, data: any) {
    return await prisma.field.updateMany({ where: { uuid: id }, data });
  }

  static async remove(id: string) {
    // Soft Delete (Metadata)
    return await prisma.field.updateMany({ 
      where: { uuid: id }, 
